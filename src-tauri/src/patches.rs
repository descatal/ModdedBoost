...
#[tauri::command]
pub async fn check_patch_activated(patch_path: &str) -> Result<bool, ()> {
    ...
    match serde_parse {
        Ok(serde_parse) => {
            let entry_v1: PpuSpu = serde_yaml::from_str(
                r#"
                AddUnitSupport:
            ...
            let entry_v2: PpuSpu = serde_yaml::from_str(
                r#"         
              AddUnitSupport:
                Gundam Extreme Vs. Full Boost:
                  BLJS10250:
...
#[tauri::command]
pub async fn activate_patch(patch_path: &str) -> Result<bool, ()> {
    let path: &Path = Path::new(&patch_path);
    if !path.exists() {
    ...
    return match serde_parse {
        Ok(mut serde_parse) => {
            let new_entry: PpuSpu = serde_yaml::from_str(
                r#"
                AddUnitSupport:
                  Gundam Extreme Vs. Full Boost:
                    BLJS10250:
